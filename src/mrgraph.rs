...
use crate::errors::GraphManipulationError;
use crate::errors::GraphManipulationError::UnknownContextFailure;
...
pub struct GraphSingleton {
    pub graph      : MyGraph,                  // null-context
    pub graphs     : HashMap<String, MyGraph>, // contexted
...
impl GraphSingleton {
    ...
    pub fn get_rank_contexted(context: &str) -> Result<MeritRank, GraphManipulationError> {
        match GRAPH.lock() {
            Ok(graph) => {
                let g =
    ...
    pub fn add_node(node_name: &str) -> Result<NodeId, GraphManipulationError> {
        match GRAPH.lock() {
            Ok(mut graph) => Ok(graph.add_node_id(node_name)),
            Err(e) => Err(GraphManipulationError::MutexLockFailure(format!(
                "Mutex lock error: {}",
                e
            ))),
    ...
    pub fn add_node_id_contexted(&mut self, context: &str, node_name: &str) -> NodeId {
        if let Some(&node_id) = self.node_names.get(node_name) {
            ...
            if !self.graphs.contains_key(context) {
                self.graphs.insert(context.to_string(), MyGraph::new());
            }
